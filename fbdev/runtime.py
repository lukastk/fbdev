...
# %% auto 0
__all__ = ['NetRuntime', 'BatchRuntime']
...
class NetRuntime:
    #TODO not clear yet what abstractions this will have, so everything is in `BatchRuntime` for now.
    pass

# %% ../nbs/api/05_runtime.ipynb 9
class BatchRuntime(NetRuntime):
    ...
    def _execute_setup(self, *args, config={}, destroy_net=True, **kwargs):
        if self._net is None:
            self._net = net = Node(node_spec=self._net_spec, parent_graph_process=None)
            net.initialise()
