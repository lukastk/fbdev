...
# %% auto 0
__all__ = ['Edge', 'Node', 'print_exception_details', 'get_caller_fullname', 'get_caller_name', 'example_function',
           'CompositeComponentFactory', 'Net']
...
class Node:
    ...
    def address(self):
        if self.parent_node is not None:
            return f"{self.parent_node.address}.Node({self.id})>"
        else:
            return None
    ...
    def run(self):
        self._run_task = self._task_manager.create_task(self._run_coro())
...
def print_exception_details(exc):
    ...
    # Print the cause of the exception, if any
    cause = exc.__cause__
    if cause:
        print("\nThe above exception was the direct cause of the following exception:")
        print_exception_details(cause)
    else:
        print("No cause for this exception.")
...
def get_caller_fullname():
    ...
    # Check if the caller is within a class
    ...
    if caller_class:
        return f"{caller_class}.{caller_name}"
    else:
        return caller_name
...
def example_function():
    print("Called from:", get_caller_name())
...
class Net(Node):
    ...
    def __init__(self, node_spec:NodeSpec):
        ...
        super().__init__(node_spec, None)
        
    @property
    def id(self): return "net"
        
    @property
    def graph(self): return self.component_process.graph
    @property
    def nodes(self): return self.component_process.nodes
    @property
    def edges(self): return self.component_process.edges
    ...
    def from_graph(cls, graph:Graph, parent_process=None):
        ...
        return cls(node_spec)
    
    @classmethod
    def execute_graph(cls, graph:Graph, *args, config={}, **kwargs):
        net = cls.from_graph(graph)
        net.initialise()
        output = net.execute(*args, config, **kwargs)
        asyncio.run(net.destroy())
        return output
    ...
    async def async_execute_graph(cls, graph:Graph, *args, config={}, **kwargs):
        net = cls.from_graph(graph)
        net.initialise()
        output = await net.async_execute(*args, config=config, **kwargs)
        await net.destroy()
        return output
    
    def _execute_setup(self, *args, config={}, **kwargs):
        if self.states.running.get(): raise RuntimeError("Net is already running.")
        if not self.states.initialised.get(): raise RuntimeError("Net is not initialised.")
        
        # Fill in input args
        input = {**kwargs}
        for port_name, val in zip(self.port_specs.input.keys(), args):
            if port_name in input:
                raise ValueError(f"Multiple values provided for '{port_name}'.")
            input[port_name] = val
        missing_input_args = set(self.port_specs.input.keys()) - set(input.keys())
        if len(missing_input_args) > 0:
            raise ValueError(f"Missing values for ports {missing_input_args}.")
        
        # Check for unexpected input args
        extra_input_args = set(input.keys()) - set(self.port_specs.input.keys())
        ...
        extra_config_args = set(config.keys()) - set(self.port_specs.config.keys())
        ...
        async def input_sender():
            for port_name, val in input.items():
                await self.send_input(PortType.INPUT, port_name, Packet(val))
                
        output = {}
        async def output_receiver():
            for port_name in self.port_specs.output.keys():
                packet = await self.receive_output(PortType.OUTPUT, port_name)
        ...
        async def main():
            ...
            async def net_IO_handler():
                input_sender_task = asyncio.create_task(input_sender())
                output_receiver_task = asyncio.create_task(output_receiver())
                await asyncio.gather(input_sender_task, output_receiver_task)
            
            _, _ = await asyncio.wait(
                [
                    asyncio.create_task(net_IO_handler()),
                    asyncio.create_task(self._task_manager.wait_for_exceptions())
                ],
                return_when=asyncio.FIRST_COMPLETED
            )

            if self._task_manager.exception_occured():
                for task, e, source_trace in self._task_manager.get_exceptions():
                    raise e
    ...
    def execute(self, *args, config={}, **kwargs):
        ...
        exec_coro, output = self._execute_setup(*args, config=config, **kwargs)
