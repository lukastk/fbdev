...
class Graph:
    ...
    def graph_head_connections(self):
        return MappingProxyType({k[1:]:v for k,v in self.head_connections.items() if k[0] == 0})
    @property
    def graph_tail_connections(self):
        return MappingProxyType({k[1:]:v for k,v in self.tail_connections.items() if k[0] == 0})
    @property
    ...
    def add_node(self, node, id=None):
        if id == 0: raise RuntimeError("Node id '0' is reserved for the graph itself.")
        if id:
            node._id = id
        else:
